...
Deploy Cronicle 0.9.130 as a single-node task scheduler with a generated administrator password and persistent scheduling data.

The Deploy on Railway button is added after the published route is verified.

## What this deploys

- Cronicle `0.9.130`, built from a checksum-verified upstream release archive
...
Cronicle normalizes administrator usernames for storage by removing dashes and dots; the configured spelling remains valid at login. Cronicle administrators can execute arbitrary commands by design. Only trusted operators should receive accounts. Jobs run inside the Cronicle container as an unprivileged user but can consume service resources and access networks reachable from that service. Cronicle 0.9.127–0.9.129 update vulnerable `sanitize-html` and `nanoid` dependencies, while Node.js 22.23.2 is a security release covering multiple high- and medium-severity runtime CVEs.
...
- Release: https://github.com/jhuckaby/Cronicle/releases/tag/v0.9.130
- Documentation: https://github.com/jhuckaby/Cronicle/tree/v0.9.130/docs
- License: MIT
